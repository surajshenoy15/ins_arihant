...
function SubmarineDriver() {
  ...
  useFrame((_, delta) => {
    ...
    const maxForward = 5, maxReverse = -5
    const desiredSpeed = throttle.current >= 0
    ...
    sharedSubPosition.x += Math.cos(hdgRad) * moveAmount
    sharedSubPosition.z += Math.sin(hdgRad) * moveAmount
...
function SubMovingRig({ children }) {
  ...
  useFrame((_, delta) => {
    ...
    ref.current.rotation.set(pitchRef.current, -hdgRad, bankRef.current + oceanRoll)
...
function ExteriorCameraChase() {
  ...
  useFrame((_, delta) => {
    ...
    if (playerDriving) {
      const chaseOffset = new THREE.Vector3(-Math.cos(hdgRad) * 18, 7, -Math.sin(hdgRad) * 18)
...
// ─── Post FX — skipped entirely on Quest/mobile ───────────────────────────────
function ScenePostFX({ bloomIntensity, vignetteDarkness, periscopeMode, isInterior, detonation, alarm }) {
  const { isPresenting } = useXR()

  // Quest browser & mobile: EffectComposer uses multi-pass FBOs that fail on
  // Adreno/Mali GPUs in a browser WebGL context → pure black screen.
  // Skip ALL post-processing on these devices.
  if (IS_LOW_END || isPresenting) return null

  return (
    <EffectComposer>
      <Bloom
        intensity={bloomIntensity}
        luminanceThreshold={periscopeMode ? 0.5 : isInterior ? 0.78 : detonation ? 0.15 : 0.6}
      ...
      <ChromaticAberration
        offset={
          periscopeMode ? new THREE.Vector2(0.001, 0.001)
          : detonation  ? new THREE.Vector2(0.008, 0.008)
          : alarm       ? new THREE.Vector2(0.002, 0.002)
...
function VRButton({ setXRError }) {
  ...
  useEffect(() => {
    ...
    async function checkXR() {
      try {
        if (!navigator.xr) { if (mounted) { setSupported(false); setChecking(false) }; return }
        const ok = await navigator.xr.isSessionSupported('immersive-vr')
        if (mounted) { setSupported(ok); setChecking(false) }
      } catch (err) {
        console.error('XR support check failed:', err)
        if (mounted) { setSupported(false); setChecking(false); setXRError(`XR check failed: ${err?.message || String(err)}`) }
      }
    }
    checkXR()
  ...
  const handleEnterVR = async () => {
    try {
      setXRError(''); setEntering(true)
      await xrStore.enterVR()
    } catch (err) {
      console.error('Failed to enter VR:', err)
      const msg = `Failed to enter VR: ${err?.message || String(err)}`
      setXRError(msg); alert(msg)
    } finally {
      setEntering(false)
    }
  }

  if (checking)   return <button disabled style={vrButtonStyle}>CHECKING XR...</button>
  if (!supported) return null

  return (
    <button onClick={handleEnterVR} disabled={entering} style={vrButtonStyle}>
...
const xrErrorStyle = {
  position: 'fixed', left: 16, bottom: 16, maxWidth: 420, zIndex: 600,
  padding: '10px 12px',
  background: 'rgba(40,0,0,0.88)', color: '#ffd5d5',
...
export default function MainScene() {
  ...
  const bloomIntensity = detonation ? 2.8 : periscopeMode ? 0.4 : isInterior ? 0.18
    : lm === LIGHT_MODES.EMERGENCY ? 0.75 : lm === LIGHT_MODES.COMBAT ? 0.55 : 0.3

  const vignetteDarkness = periscopeMode ? 0.0
    : isInterior
      ? lm === LIGHT_MODES.STEALTH ? 0.45 : lm === LIGHT_MODES.OFF ? 0.55 : alarm ? 0.38 : 0.22
      : lm === LIGHT_MODES.STEALTH ? 0.82 : lm === LIGHT_MODES.OFF ? 0.9  : alarm ? 0.65 : 0.42
  ...
  return (
    <>
      ...
      <Canvas
        gl={{
          antialias: !IS_LOW_END,          // false on Quest/mobile — MSAA kills Adreno GPU
          alpha: false,
          ...
          xrCompatible: true,
          // Explicitly request WebGL2 — Quest browser prefers it
          failIfMajorPerformanceCaveat: false,
        }}
        // Quest 2 = 1.5 max, Quest 3 = 2.0 max, desktop = full native
        dpr={IS_QUEST ? [1, 1.5] : IS_MOBILE ? [1, 2] : [1, window.devicePixelRatio]}
        camera={{ fov: cameraFov, near: 0.05, far: 600, position: cameraPos }}
        shadows={false}                    // no shadows at Canvas level — handled per-light
        style={{ position: 'fixed', inset: 0, background: '#000810' }}
        onCreated={({ gl }) => {
          // Do NOT manually set gl.xr.enabled here — @react-three/xr manages this
          // internally. Setting it manually causes a race condition on Quest browser
          // that results in a black screen.
