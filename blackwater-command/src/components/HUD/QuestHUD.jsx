 ...
 * Also fixes VR entry and AI voice input via touch
 */
...
function Btn({ label, icon, variant = 'default', active = false, onPress, w, h = 48, size = 11 }) {
  ...
  const base = variant === 'red' ? BTN_RED : variant === 'yellow' ? BTN_YELLOW
    : variant === 'green' ? BTN_GREEN : variant === 'purple' ? BTN_PURPLE : BASE_BTN
  ...
  return (
    <div onPointerDown={fire} style={{
      ...base,
      minHeight: h, width: w || 'auto',
      fontSize: size,
      background: active || down ? 'rgba(0,229,255,0.15)' : base.background,
      borderColor: active || down ? 'rgba(0,229,255,0.9)' : base.border?.replace?.('1px solid ', '') || 'rgba(0,229,255,0.3)',
...
// ─── D-Pad for driving ────────────────────────────────────────────────────────
function DrivePad() {
  const fireKey = useCallback((code, down) => {
    const evt = new KeyboardEvent(down ? 'keydown' : 'keyup', { code, bubbles: true, cancelable: true })
    window.dispatchEvent(evt)
  }, [])

  const hold = useCallback((code) => {
    fireKey(code, true)
    const onUp = () => { fireKey(code, false); window.removeEventListener('pointerup', onUp); window.removeEventListener('pointercancel', onUp) }
    window.addEventListener('pointerup', onUp)
    window.addEventListener('pointercancel', onUp)
  }, [fireKey])

  const DBtn = ({ code, icon, style }) => (
    <div onPointerDown={(e) => { e.preventDefault(); hold(code) }} style={{
      ...BASE_BTN, width: 52, height: 52, borderRadius: 8,
      fontSize: 20, ...style,
    }}>{icon}</div>
  )
  ...
  return (
    <div style={{ display: 'grid', gridTemplateColumns: '52px 52px 52px', gridTemplateRows: '52px 52px 52px', gap: 4 }}>
      <div /><DBtn code="ArrowUp"    icon="▲" /><div />
      <DBtn code="ArrowLeft" icon="◀" />
      <div style={{ ...BASE_BTN, width: 52, height: 52, borderRadius: 8, fontSize: 9, color: 'rgba(0,229,255,0.4)' }}>HELM</div>
      <DBtn code="ArrowRight" icon="▶" />
      <div /><DBtn code="ArrowDown" icon="▼" /><div />
    </div>
  )
}
...
function DepthPad() {
  const hold = useCallback((code) => {
    const fire = (down) => window.dispatchEvent(new KeyboardEvent(down ? 'keydown' : 'keyup', { code, bubbles: true }))
    fire(true)
    const up = () => { fire(false); window.removeEventListener('pointerup', up); window.removeEventListener('pointercancel', up) }
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
      <div onPointerDown={(e) => { e.preventDefault(); hold('KeyE') }} style={{ ...BTN_YELLOW, width: 52, height: 52, borderRadius: 8, fontSize: 20 }}>▲</div>
      <div style={{ ...BASE_BTN, width: 52, height: 24, borderRadius: 4, fontSize: 8, color: 'rgba(0,229,255,0.4)', minHeight: 'unset', padding: 0 }}>DEPTH</div>
      <div onPointerDown={(e) => { e.preventDefault(); hold('KeyQ') }} style={{ ...BTN_YELLOW, width: 52, height: 52, borderRadius: 8, fontSize: 20 }}>▼</div>
    </div>
  )
}
...
function AIVoicePanel() {
  ...
  const startListening = useCallback(() => {
    const SR = window.SpeechRecognition || window.webkitSpeechRecognition
    if (!SR) {
      setLastReply('Speech recognition not available on this browser.')
      return
    }
    if (listening) {
    ...
    r.onerror  = (e) => { setListening(false); setTranscript(''); setLastReply(`Error: ${e.error}`) }

    r.onresult = (e) => {
      // Pick best alternative
      const results = Array.from(e.results[0]).map(alt => alt.transcript)
      const best = results[0] || ''
      setTranscript(best)

      // Intercept reply — wrap indraVoice.speak to capture text
      const orig = indraVoice.speak.bind(indraVoice)
      indraVoice.speak = (text, priority) => {
        setLastReply(text)
        orig(text, priority)
        // Restore after first call
        setTimeout(() => { indraVoice.speak = orig }, 100)
      }
  ...
  // Text input fallback (for Quest where speech may not work)
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
      <div style={{ ...BASE_BTN, fontSize: 9, color: 'rgba(0,229,255,0.5)', justifyContent: 'flex-start', minHeight: 20, padding: '2px 8px', border: 'none', background: 'none' }}>
        ◈ INDRA AI ASSISTANT
      </div>
      ...
      <Btn
        icon={listening ? '🔴' : '🎙️'}
        label={listening ? 'LISTENING...' : supported ? 'SPEAK TO INDRA' : 'VOICE N/A'}
      ...
      <div style={{ display: 'flex', gap: 4 }}>
        ...
        />
        <Btn icon="⮕" onPress={submitText} w={44} h={42} />
      </div>

      {/* Transcript + reply */}
      {transcript && (
        <div style={{ fontFamily: FONT, fontSize: 9, color: 'rgba(0,229,255,0.5)', padding: '4px 8px',
          background: 'rgba(0,229,255,0.05)', borderRadius: 4, lineHeight: 1.5 }}>
          YOU: {transcript}
        </div>
      )}
      {lastReply && (
        <div style={{ fontFamily: FONT, fontSize: 9, color: '#ffe066', padding: '4px 8px',
          background: 'rgba(255,224,102,0.06)', borderRadius: 4, lineHeight: 1.5, maxWidth: '100%',
...
export default function QuestHUD() {
  const [tab, setTab] = useState('drive')   // drive | tactical | weapons | lights | ai
  ...
  const tabStyle = (t) => ({
    ...
    border: 'none', borderBottom: tab === t ? '2px solid #00e5ff' : '2px solid rgba(0,229,255,0.1)',
    letterSpacing: 0.5,
  })
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        background: 'rgba(0,4,16,0.95)', borderTop: '1px solid rgba(0,229,255,0.2)',
        ...
        {/* View toggle — most important button */}
      ...
      {/* ── Main panel ── */}
      {expanded && (
        <div style={{ background: 'rgba(0,4,16,0.97)', borderTop: '1px solid rgba(0,229,255,0.15)' }}>
          ...
          <div style={{ padding: 10, minHeight: 140 }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                ...
                <div style={{ maxHeight: 80, overflowY: 'auto', marginBottom: 4 }}>
                  {contacts.slice(0, 6).map(c => (
                    <div key={c.id} onPointerDown={(e) => { e.preventDefault(); useGameStore.getState().trackContact(c.id); speakReactive('contactTracked') }}
            ...
            {tab === 'lights' && (
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8 }}>
                ...
                <Btn label="SPOTLIGHT" onPress={() => { useGameStore.getState().toggleSpotlight(); speakReactive('spotlightOn') }} active={useGameStore.getState()?.spotlightOn} variant="yellow" h={48} />
              </div>
            )}
...
// ─── Sub components used inside QuestHUD ─────────────────────────────────────
function DrivePad() {
  const hold = useCallback((code) => {
    const fire = (down) => window.dispatchEvent(new KeyboardEvent(down ? 'keydown' : 'keyup', { code, bubbles: true, cancelable: true }))
    fire(true)
    const up = () => { fire(false); window.removeEventListener('pointerup', up); window.removeEventListener('pointercancel', up) }
    window.addEventListener('pointerup', up)
    window.addEventListener('pointercancel', up)
...
function DepthPad() {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 4, flexShrink: 0 }}>
      <div onPointerDown={(e) => { e.preventDefault(); hold('KeyE') }} style={{ ...BTN_YELLOW, width: 52, height: 52, borderRadius: 8, fontSize: 22, flexShrink: 0 }}>↑</div>
      <div style={{ fontFamily: FONT, fontSize: 8, color: 'rgba(0,229,255,0.35)', textAlign: 'center', padding: '2px 0' }}>DEPTH</div>
      <div onPointerDown={(e) => { e.preventDefault(); hold('KeyQ') }} style={{ ...BTN_YELLOW, width: 52, height: 52, borderRadius: 8, fontSize: 22, flexShrink: 0 }}>↓</div>
    </div>
  )
}
...
function AIVoicePanel() {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
      <Btn icon={listening ? '🔴' : '🎙️'} label={listening ? 'LISTENING — TAP TO STOP' : supported ? 'TAP TO SPEAK TO INDRA' : 'VOICE UNAVAILABLE'} variant={listening ? 'red' : 'default'} active={listening} onPress={startListening} h={50} />
      <div style={{ display: 'flex', gap: 6 }}>
        <input value={textInput} onChange={e => setTextInput(e.target.value)}
          onKeyDown={e => e.key === 'Enter' && submitText()}
          placeholder="Or type a command / question..."
          style={{ flex: 1, fontFamily: FONT, fontSize: 11, background: 'rgba(0,8,22,0.92)',
            border: '1px solid rgba(0,229,255,0.25)', borderRadius: 6, color: '#00e5ff',
            padding: '0 10px', height: 42, outline: 'none', boxSizing: 'border-box' }} />
        <div onPointerDown={(e) => { e.preventDefault(); submitText() }} style={{ ...BASE_BTN, width: 44, height: 42, minHeight: 'unset', borderRadius: 6, fontSize: 18 }}>⮕</div>
      </div>
      {transcript && (
        <div style={{ fontSize: 9, color: 'rgba(0,229,255,0.55)', padding: '4px 8px',
          background: 'rgba(0,229,255,0.05)', borderRadius: 4, lineHeight: 1.6 }}>
          YOU: {transcript}
        </div>
      )}
      {lastReply && (
        <div style={{ fontSize: 9, color: '#ffe066', padding: '4px 8px',
          background: 'rgba(255,224,102,0.06)', borderRadius: 4, lineHeight: 1.6, wordBreak: 'break-word' }}>
          INDRA: {lastReply}
        </div>
      )}
